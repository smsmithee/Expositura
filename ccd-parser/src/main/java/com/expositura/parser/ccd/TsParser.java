...
/**
 * Parses a TS datatype XML element to the java object or from the object to the xml
 * 
 * @author Sean Smith
 */
public class TsParser {
  
  public static Ts fromXml(final Node node) {
    final Ts ts = new Ts();
    ...
    if (attributes != null && attributes.getLength() > 0) {
      ...
      if (nullFlavor != null) {
        ts.setNullFlavor(nullFlavor.getNodeValue());
      }
      
      // value
      final Node value = attributes.getNamedItem("value");
      if (value != null) {
        ts.setValue(value.getNodeValue());
      }
      
    }
    
    return Ts.isEmpty(ts) ? null : ts;
  }
