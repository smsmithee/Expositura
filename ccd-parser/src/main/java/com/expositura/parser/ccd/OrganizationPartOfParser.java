...
import com.expositura.model.ccd.OrganizationPartOf;
import org.w3c.dom.NamedNodeMap;
import org.w3c.dom.Node;
import org.w3c.dom.NodeList;

/**
 * Parses the OrganizationPartOf XML element to the java object or from the object to the xml
 * 
 * @author Sean Smith
 */
public class OrganizationPartOfParser {
  
  public static OrganizationPartOf fromXml(final Node node) {
    final OrganizationPartOf organizationPartOf = new OrganizationPartOf();
    ...
    if (attributes != null && attributes.getLength() > 0) {
      ...
      if (nullFlavor != null) {
        organizationPartOf.setNullFlavor(nullFlavor.getNodeValue());
      }
      
      // classCode
      final Node classCode = attributes.getNamedItem("classCode");
      if (classCode != null) {
        organizationPartOf.setClassCode(classCode.getNodeValue());
      }
    ...
    for (int i = 0; i < nodeList.getLength(); i++) {
      ...
      if (child.hasAttributes() || child.hasChildNodes()) {
        switch (child.getNamespaceURI() + "|" + child.getLocalName()) {
          case "urn:hl7-org:v3|realmCode" -> organizationPartOf.addRealmCode(CsParser.fromXml(child));
          ...
          case "urn:hl7-org:sdtc|identifiedBy" -> organizationPartOf.addSdtcIdentifiedBy(IdentifiedByParser.fromXml(child));
          ...
          case "urn:hl7-org:v3|wholeOrganization" -> organizationPartOf.setWholeOrganization(OrganizationParser.fromXml(child));
        }
      }
    }
    
    return OrganizationPartOf.isEmpty(organizationPartOf) ? null : organizationPartOf;
  }
