...
import com.expositura.model.ccd.Pq;
import org.w3c.dom.NamedNodeMap;
import org.w3c.dom.Node;
import org.w3c.dom.NodeList;

/**
 * Parses a PQ datatype XML element to the java object or from the object to the xml
 * 
 * @author Sean Smith
 */
public class PqParser {
  
  public static Pq fromXml(final Node node) {
    final Pq pq = new Pq();
    ...
    if (attributes != null && attributes.getLength() > 0) {
      ...
      if (nullFlavor != null) {
        pq.setNullFlavor(nullFlavor.getNodeValue());
      }
      
      // unit
      final Node unit = attributes.getNamedItem("unit");
      if (unit != null) {
        pq.setUnit(unit.getNodeValue());
      }
      
      // value
      final Node value = attributes.getNamedItem("value");
      if (value != null) {
        pq.setValue(value.getNodeValue());
      }
    ...
    for (int i = 0; i < nodeList.getLength(); i++) {
      ...
      if (child.hasAttributes() || child.hasChildNodes()) {
        switch (child.getNamespaceURI() + "|" + child.getLocalName()) {
          case "urn:hl7-org:v3|translation" -> pq.addTranslation(PqrParser.fromXml(child));
        }
      }
    }
    
    return Pq.isEmpty(pq) ? null : pq;
  }
