...
import com.expositura.model.ccd.PatientRole;
import org.w3c.dom.NamedNodeMap;
import org.w3c.dom.Node;
import org.w3c.dom.NodeList;

/**
 * Parses the PatientRole XML element to the java object or from the object to the xml
 * 
 * @author Sean Smith
 */
public class PatientRoleParser {
  
  public static PatientRole fromXml(final Node node) {
    final PatientRole patientRole = new PatientRole();
    ...
    if (attributes != null && attributes.getLength() > 0) {
      ...
      if (nullFlavor != null) {
        patientRole.setNullFlavor(nullFlavor.getNodeValue());
      }
      
      // classCode
      final Node classCode = attributes.getNamedItem("classCode");
      if (classCode != null) {
        patientRole.setClassCode(classCode.getNodeValue());
      }
    ...
    for (int i = 0; i < nodeList.getLength(); i++) {
      ...
      if (child.hasAttributes() || child.hasChildNodes()) {
        switch (child.getNamespaceURI() + "|" + child.getLocalName()) {
          case "urn:hl7-org:v3|realmCode" -> patientRole.addRealmCode(CsParser.fromXml(child));
          ...
          case "urn:hl7-org:sdtc|identifiedBy" -> patientRole.addSdtcIdentifiedBy(IdentifiedByParser.fromXml(child));
          ...
          case "urn:hl7-org:v3|providerOrganization" -> patientRole.setProviderOrganization(OrganizationParser.fromXml(child));
        }
      }
    }
    
    return PatientRole.isEmpty(patientRole) ? null : patientRole;
  }
