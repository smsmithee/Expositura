...
import com.expositura.model.ccd.Cv;
import org.w3c.dom.NamedNodeMap;
import org.w3c.dom.Node;
import org.w3c.dom.NodeList;

/**
 * Parses a CV datatype XML element to the java object or from the object to the xml
 * 
 * @author Sean Smith
 */
public class CvParser {
  
  public static Cv fromXml(final Node node) {
    final Cv cv = new Cv();
    ...
    for (int i = 0; i < nodeList.getLength(); i++) {
      ...
      if (child.hasAttributes() || child.hasChildNodes()) {
        switch (child.getNamespaceURI() + "|" + child.getLocalName()) {
          case "urn:hl7-org:v3|originalText" -> cv.setOriginalText(EdParser.fromXml(child));
        }
      }
    }
    
    return Cv.isEmpty(cv) ? null : cv;
  }
